...
public final class AnnotatedApplication extends AbstractDefaultApplication {
	...
	private static void fillHandlersFromMethod(final Map<String, ResourceHandler> handlers,
        ...
        fillHandlersFromVerbMethod(handlers, applicationInstance, method, EXECUTE.class, "EXEC");
        fillHandlersFromVerbMethod(handlers, applicationInstance, method, GET.class, "GET");
        fillHandlersFromVerbMethod(handlers, applicationInstance, method, PUT.class, "PUT");
        fillHandlersFromVerbMethod(handlers, applicationInstance, method, POST.class, "POST");
        fillHandlersFromVerbMethod(handlers, applicationInstance, method, DELETE.class, "DEL");
    }

	private static void fillHandlersFromVerbMethod(final Map<String, ResourceHandler> handlers, final Object applicationInstance, final Method method, final Class<? extends Annotation> annotationClazz,
	...
	private static void fillHandlersFromResourceMethod(final Map<String, ResourceHandler> handlers,
        ...
        for (int i = 0; i < name.length(); i++) {
            ...
            if (verbBuilder != null) {
                if (Character.isUpperCase(c)) {
                    verb = StringUtils.upperCase(verbBuilder.toString());
        ...
        if (verb == null || StringUtils.isEmpty(verb) || resource == null || StringUtils.isEmpty(resource)) {
	...
	private static void mapMethod(final Map<String, ResourceHandler> handlers, final Object applicationInstance,
        ...
        handlers.put(new StringBuilder().append(verb).append("/").append(resource).toString(), new PlainMethodHandler(mh.bindTo(applicationInstance)));
    }
