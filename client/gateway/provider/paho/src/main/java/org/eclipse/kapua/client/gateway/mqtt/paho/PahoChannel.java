...
public class PahoChannel extends AbstractMqttChannel {
	...
	private void handleResubscribe() {
        subscriptions.entrySet().forEach((final Map.Entry<String, MqttMessageHandler> entry) -> internalSubscribe(entry.getKey())
				.whenComplete((value, ex) -> logger.warn("Failed to re-subscribe to '{}'", entry.getKey(), ex)));
    }
    ...
    protected void unsubscribeMqtt(final Set<String> mqttTopics) throws MqttException {
        ...
        synchronized (this) {
            topics.addAll(mqttTopics.stream().filter(topic -> subscriptions.remove(topic) != null).collect(Collectors.toList()));
        }
